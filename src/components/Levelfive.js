...
const Levelone = ({ username, rollnum, initialScore, timeLeft }) => {
  ...
  const handleCastSpell = () => {
    const dummyPassword = 'botnet'; // Dummy password for testing
  ...
  const handleNextLevel = async () => {
    ...
    try {
      ...
      const fetchResponse = await fetch(`https://json-production-72ef.up.railway.app/books?rollnum=${rollnum}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (fetchResponse.ok) {
        ...
        if (existingDataArray && existingDataArray.length > 0) {
          ...
          if (currentLevel === existingData.level + 1) {
            ...
            const updateResponse = await fetch(`https://json-production-72ef.up.railway.app/books/${existingData.id}`, {
          ...
          <div className="popup-content">
            ...
            {isSpellValidated ? (
              ...
              <div className='column'>
                ...
                A network of compromised computers remotely controlled to perform malicious activities.
                </h2>
