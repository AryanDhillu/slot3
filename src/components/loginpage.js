...
const LoginPage = ({ startTimer, setUserInfo }) => {
  ...
  const checkRollNoExists = async (rollNo) => {
    try {
      const response = await fetch(`https://json-production-72ef.up.railway.app/books?rollnum=${rollNo}`);
  ...
  const submitUserData = async () => {
    ...
    try {
      const response = await fetch('https://json-production-72ef.up.railway.app/books', {
        method: 'POST',
        headers: {
